...
-- Ported from the reference web app (Ammar-Sagheer/Petrol-Pump-Management-Software,
-- supabase/migrations/001 + the corrections later migrations made to it:
-- starting_reading (012), the corrected unit/fuel layout (013), and the tank
-- capacity guard (017) are folded in here from the start, since this is a fresh
-- database with no pre-existing rows to reconcile.
--
...
-- ---------------------------------------------------------------------------
-- pump_today() - "today" as defined by where the pump is, not by the server's
-- clock. The reference app hit a real bug from this: Postgres current_date is
-- evaluated in the database's own timezone, which disagreed with the app's
-- Asia/Karachi definition of the business day between midnight and 5am local.
-- Defined this early because tanks.opening_stock_date defaults to it below.
-- Change this one line (and PUMP_TIMEZONE in app/_lib/date-helpers.js) if the
-- pump ever moves to another timezone.
-- ---------------------------------------------------------------------------
...
comment on function public.pump_today() is
  'Today at the pump. Use instead of current_date, which is the server''s own timezone.';

-- ---------------------------------------------------------------------------
-- profiles - one row per login, holding both the role AND the credential.
--
-- In the reference (Supabase) app this table only held the role; the login
-- itself - email, password hash - lived in Supabase Auth's own auth.users
-- table, managed by GoTrue. There is no GoTrue here, so profiles IS the
-- identity table: email and password_hash live on it directly. See
-- 002_identity_and_sessions.sql for how a password is checked and a session
-- issued without ever handing the hash to the application layer.
-- ---------------------------------------------------------------------------
...
comment on table public.profiles is
  'Login, credential and role in one row. Deactivate with is_active = false '
  'rather than deleting, so past entries keep their author.';

-- ---------------------------------------------------------------------------
-- tanks - the underground storage. current_stock_litres is a cached book stock
-- kept in sync by triggers (see 003); it is always recomputed from scratch, so
...
create table public.tanks (
  ...
  -- A tank cannot hold more than it holds. Only opening stock is constrained;
  -- current_stock_litres is written by the stock triggers and deliberately
  -- left unconstrained, so a mistyped delivery is a number someone can see and
  -- correct, not a failed day's entry.
  constraint tanks_opening_stock_within_capacity
...
--
-- starting_reading: the meter reading on the day this pump began using the
-- app. Used as the opening figure on a nozzle's very first entry, before any
-- previous day's closing exists to fall back on.
-- ---------------------------------------------------------------------------
create table public.nozzles (
  ...
  starting_reading numeric(12, 2) not null default 0 check (starting_reading >= 0),
  is_active        boolean not null default true,
  created_at       timestamptz not null default now(),
...
-- A trigger (003) posts each of these into the customer's ledger automatically.
-- ---------------------------------------------------------------------------
...
-- Enforced two ways: no update/delete RLS policies, and a hard trigger guard
-- (003) that even a direct SQL session cannot get past.
--
-- credit_sale_id and created_by both use ON DELETE SET NULL rather than
-- RESTRICT: deleting the reading behind a credit slip, or deleting the staff
-- login that wrote an entry, must not be blocked by money that is meant to stay
-- append-only. The append-only trigger carries a narrow exception for exactly
-- these two columns going to null - see 003.
-- ---------------------------------------------------------------------------
create table public.ledger_entries (
  ...
  credit_sale_id uuid unique references public.credit_sales (id) on delete set null,
  created_by     uuid references public.profiles (id) on delete set null,
  created_at     timestamptz not null default now(),
