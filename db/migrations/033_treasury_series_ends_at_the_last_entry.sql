-- =============================================================================
-- 033_treasury_series_ends_at_the_last_entry.sql
--
-- Ported from the reference app's 046_treasury_series_ends_at_the_last_entry.sql,
-- unchanged in logic.
--
-- The day-by-day chart drew a flat tail from the last entry to today.
--
-- 032 ended the window at `greatest(pump_today(), max(entry_date))` and carried
...
-- sound for a GAP - a quiet day between two busy ones is a real day the safe
-- sat there - and wrong for the END of the sheet, which is where it was also
-- being applied.
--
...
-- at Rs X" when the truth is "nobody has written today down".
--
...
--
-- The two movement tiles read from the same window, so "the last N days" now
-- means the N days up to the last entry rather than up to today. The page says
-- so in as many words - "N days to <date>" - rather than leaving a window that
-- has quietly stopped moving described as if it had not.
-- =============================================================================
...
comment on function public.treasury_overview(integer) is
  ...
  'over the last p_days ending at the LAST ENTRY, not at today - see 033.';
