-- =============================================================================
-- 034_treasury_a_page_is_a_day.sql
--
-- Ported from the reference app's 047_treasury_a_page_is_a_day.sql, unchanged
-- in logic (its own grant already targets the identity model this repo uses -
-- authenticated there is app_user here, applied below).
--
...
-- and a bit days, cut mid-day at both ends. Nothing about "25" means anything
...
-- app's existing `DateJump` box already navigates by.
--
...
-- arrows never land on an empty page.
--
-- A REQUESTED DATE ALWAYS RESOLVES TO A DAY THAT EXISTS. Ask for a day with no
-- entries and this answers with the nearest day at or before it, falling back
-- to the earliest day there is. The page then says which day it actually
-- landed on.
-- =============================================================================
...
comment on function public.treasury_day(date) is
  ...
  'have entries - so paging never lands on an empty day. See 034.';

revoke all on function public.treasury_day(date) from public;
grant execute on function public.treasury_day(date) to app_user;
