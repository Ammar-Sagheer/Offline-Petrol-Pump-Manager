-- =============================================================================
-- 037_clear_the_old_activity_log.sql
--
-- Letting the owner throw away the OLD end of the audit trail.
--
-- WHY, WHEN 024 SAID THE LOG IS APPEND-ONLY FOR EVERYBODY.
--
...
--     approach as `app.purging_customer` in migration 022, and for the same
--     reason: a stray DELETE from server code or a later refactor meets the
--     refusal it always did, because none of them set it.
--
-- Ported from the reference app's 050_clear_the_old_activity_log.sql. The
-- usual two edits and nothing else: policies and grants target app_user
-- rather than `authenticated`, and there is no anon role to revoke from.
-- =============================================================================
...
--
-- Restated in full rather than diffed against 024: `create or replace` has to
...
revoke execute on function public.activity_log_cutoff(int)      from public;
revoke execute on function public.activity_log_trim_counts()    from public;
revoke execute on function public.clear_activity_log(int)       from public;
grant  execute on function public.activity_log_cutoff(int)      to app_user;
grant  execute on function public.activity_log_trim_counts()    to app_user;
grant  execute on function public.clear_activity_log(int)       to app_user;
