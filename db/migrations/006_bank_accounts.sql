...
-- month's Excel export (get_month_export, 008), written before the rows age out.
--
-- Ported unchanged from the reference app's 018_bank_accounts.sql - nothing
-- here touches identity, so there is nothing to rewrite.
-- =============================================================================
...
create policy "bank accounts: super admin only"
  on public.bank_accounts for all to app_user
  using (public.is_super_admin())
  with check (public.is_super_admin());

create policy "bank transactions: super admin only"
  on public.bank_transactions for all to app_user
  using (public.is_super_admin())
  with check (public.is_super_admin());

grant select, insert, update, delete on public.bank_accounts     to app_user;
grant select, insert, update, delete on public.bank_transactions to app_user;
grant select                         on public.bank_account_balances to app_user;
