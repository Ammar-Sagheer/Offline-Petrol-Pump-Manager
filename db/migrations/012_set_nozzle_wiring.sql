...
--
-- WHY AN RPC AND NOT SIX UPDATES FROM THE APP. Same reason
-- record_bank_payment() exists: six statements sent one at a time can fail
-- after the third, leaving half the nozzles pointing at the new tanks and
-- half at the old. That is worse than not saving at all, because nothing on
-- screen says which half took - and tank_id decides which tank a sale draws
-- down, so a half-applied change quietly drains the wrong tank. As one
-- UPDATE it is all six or none.
-- =============================================================================
...
revoke execute on function public.set_nozzle_wiring(jsonb) from public;
grant  execute on function public.set_nozzle_wiring(jsonb) to app_user;
