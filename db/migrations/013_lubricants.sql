...
--
-- Ported from the reference app's 024_lubricants.sql. Only the identity
-- plumbing changed, same as everywhere else here: auth.uid() -> current_uid(),
-- and policies/grants target app_user rather than Supabase's `authenticated`
-- (see 004's "ONE POSTGRES ROLE, NOT THREE"). Every table, constraint, trigger
-- and RPC below is byte-for-byte the same logic as the web app's.
--
...
-- migration 011: what the customer paid, and what the invoice says, are the
...
-- exception as created_by and credit_sale_id in migration 003: a foreign key
...
-- Removing a sale
--
-- Same treatment as deleting a nozzle reading (008's delete_reading): reverse
-- the ledger first, delete second, both in one transaction. Deleting the sale
-- on its own would release the ledger entry's pointer and leave the customer
...
create policy "lubricants: staff read"
  on public.lubricants for select to app_user
  using (public.is_active_staff());

create policy "lubricants: super admin writes"
  on public.lubricants for all to app_user
  using (public.is_super_admin())
  with check (public.is_super_admin());

create policy "lubricant purchases: staff read"
  on public.lubricant_purchases for select to app_user
  using (public.is_active_staff());

create policy "lubricant purchases: staff create"
  on public.lubricant_purchases for insert to app_user
  with check (public.is_active_staff());

create policy "lubricant purchases: super admin updates"
  on public.lubricant_purchases for update to app_user
  using (public.is_super_admin())
  with check (public.is_super_admin());

create policy "lubricant purchases: super admin deletes"
  on public.lubricant_purchases for delete to app_user
  using (public.is_super_admin());

create policy "lubricant sales: staff read"
  on public.lubricant_sales for select to app_user
  using (public.is_active_staff());

create policy "lubricant sales: staff create"
  on public.lubricant_sales for insert to app_user
  with check (public.is_active_staff());

create policy "lubricant sales: super admin updates"
  on public.lubricant_sales for update to app_user
  using (public.is_super_admin())
  with check (public.is_super_admin());

create policy "lubricant sales: super admin deletes"
  on public.lubricant_sales for delete to app_user
  using (public.is_super_admin());

grant select, insert, update, delete on public.lubricants          to app_user;
grant select, insert, update, delete on public.lubricant_purchases to app_user;
grant select, insert, update, delete on public.lubricant_sales     to app_user;

-- ---------------------------------------------------------------------------
-- Function permissions. Same rule as 004: revoke the default PUBLIC grant on
-- everything, grant back to app_user only what the app actually calls. Trigger
-- functions are never called directly and get no grant at all - they run under
-- their own trigger context regardless.
-- ---------------------------------------------------------------------------
revoke execute on function public.trg_post_lubricant_sale_to_ledger()   from public;
revoke execute on function public.trg_recalc_lubricant_from_row()       from public;
revoke execute on function public.trg_recalc_lubricant_from_product()   from public;
revoke execute on function public.recalc_lubricant_stock(uuid)          from public;

revoke execute on function public.calculate_lubricant_stock(uuid, date) from public;
revoke execute on function public.get_lubricant_stock(date)             from public;
revoke execute on function public.get_lubricant_day(date)               from public;
revoke execute on function public.delete_lubricant_sale(uuid)           from public;
revoke execute on function public.delete_lubricant(uuid)                from public;

grant execute on function public.calculate_lubricant_stock(uuid, date) to app_user;
grant execute on function public.get_lubricant_stock(date)             to app_user;
grant execute on function public.get_lubricant_day(date)               to app_user;
grant execute on function public.delete_lubricant_sale(uuid)           to app_user;
grant execute on function public.delete_lubricant(uuid)                to app_user;
