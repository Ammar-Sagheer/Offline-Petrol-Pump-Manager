-- =============================================================================
-- 036_the_database_works_out_the_cash.sql
--
...
--
-- One paisa. Every figure on the screen was correct, and no amount of
-- re-typing could have fixed it.
--
...
-- all - a whole number times a two-decimal rate has at most two decimals -
-- which is why entering the reading "without points" worked.
--
...
--
-- `p_cash` stays in the signature and is ignored - dropping it would be a
-- pointless breaking change to a function this app calls with named arguments
-- nowhere else does.
--
-- Ported from the reference app's 052_the_database_works_out_the_cash.sql.
-- The only edits are the ones every migration here carries: `auth.uid()`
-- becomes `public.current_uid()`, `security invoker` and the grant to
-- `app_user` rather than `authenticated` - both already true of this
-- function since 008, so this changes only the arithmetic inside it.
-- `format-helpers.js`'s `saleAmount()` - the JavaScript half of this fix,
-- used by the reading and lubricant-sale screens so what is on screen is
-- what is about to be saved - travels with it in the same round of work.
-- =============================================================================
...
begin
  ...
  values (
    ...
    p_rate, v_cash, v_credit_total, public.current_uid()
  )
...
comment on function public.create_nozzle_reading(uuid, date, numeric, numeric, numeric, numeric, jsonb) is
  ...
  'in numeric - p_cash is accepted for compatibility and ignored. See 036.';

-- The grants a `create or replace` keeps, restated so this file can be read on
-- its own: replacing a function does not drop its ACL, but 008 is a long way
