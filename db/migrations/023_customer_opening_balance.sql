-- =============================================================================
-- 023_customer_opening_balance.sql
--
-- Ported from the reference app's 034_customer_opening_balance.sql. Two edits:
-- auth.uid() -> current_uid() for the actor on the opening entry, and the grant
-- at the bottom targeting app_user (see 004).
--
-- AND ONE FIX, which the reference app needs too. Its entry_type value is
--
--     case when p_opening_direction = 'owes' then 'debit' else 'credit' end
--
-- and every call with an opening balance raises
--
--     column "entry_type" is of type ledger_entry_type but expression is of
--     type text
--
-- A bare 'debit' is an untyped literal that Postgres coerces to the enum on
-- assignment, but a CASE whose branches are all untyped literals resolves to
-- text first, and there is no implicit text -> enum cast. So the whole function
-- fails at its one useful moment: creating a customer who arrives owing money.
-- Confirmed against a real cluster, not reasoned about - a bare literal is
-- accepted, the CASE is refused, and the CASE with the cast below is accepted.
-- The cast is the smallest possible change and does not alter what is stored.
--
...
begin
  ...
  -- Whole rupees, like every other ledger write since 021.
  ...
  if v_amount > 0 then
    ...
    values (
      v_id,
      -- The cast is load-bearing - see the header.
      (case when p_opening_direction = 'owes' then 'debit' else 'credit' end)
