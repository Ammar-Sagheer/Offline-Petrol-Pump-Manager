...
--
-- Ported from the reference app's 026_readings_may_not_overlap.sql. Nothing
-- needed translating: no auth.uid(), no RLS, no Supabase roles - just a trigger
-- and plain Postgres. The one addition is the revoke at the bottom, which is
-- this repo's convention for a trigger function (see 004).
--
...
-- Trigger functions are never called directly - 004's convention.
revoke execute on function public.check_reading_does_not_overlap() from public;
