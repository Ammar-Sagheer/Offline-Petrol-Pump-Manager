-- =============================================================================
-- 035_profit_counts_stock_sold.sql
--
-- Ported verbatim from the reference app's 049_profit_counts_stock_sold.sql -
-- plain Postgres throughout, nothing here touches identity or RLS, so nothing
-- needed to change porting it. Only the grants target app_user, as usual.
--
-- Profit was counting stock BOUGHT instead of stock SOLD.
--
-- All three reporting RPCs computed
--
--     profit = sales - purchases - expenses
--
-- with no opening or closing stock in it anywhere, so a month was charged for
-- every litre that ARRIVED in it whether or not any of it was sold. A month
-- ending with fuel in the tank is understated by the value of that fuel; a
-- month that runs the tanks down is OVERSTATED, because it sells stock a
-- previous month already paid for. The errors cancel over years and never
...
-- in a rising market an average that still carries old cheap deliveries values
-- today's full tank below what it cost, and understates profit for exactly as
-- long as prices keep rising. Weighting by what is physically left has no such
-- drift, and for a tank it is also the truth: the fuel in there IS the last
-- few loads.
--
-- STOCK THE RECORDS DO NOT REACH. Tanks are given an opening quantity in
-- Settings when the pump joins the app, with no delivery behind it and
-- therefore no cost. Litres not covered by recorded purchases are valued at
-- that tank's all-time average purchase rate, which is an estimate and is
-- flagged as one. It affects only the first month that has purchases; every
-- month after opens on stock the app watched arrive.
--
-- LUBRICANTS GET THE SAME TREATMENT, for the same reason.
--
...
-- suppliers against, and it is what Treasury and Banking move. It is simply
-- no longer subtracted from sales to make profit.
-- =============================================================================
...
comment on function public.tank_stock_value(uuid, date) is
  ...
  'average cost of the deliveries it is actually made of. See migration 035.';
...
comment on function public.lubricant_stock_value(uuid, date) is
  ...
  'tank_stock_value. See migration 035.';
...
comment on function public.cost_of_goods_sold(date, date) is
  ...
  'what was actually SOLD, which is what profit subtracts. See migration 035.';
...
--
-- PATCHED RATHER THAN REPRODUCED IN FULL, matching the reference migration's
-- own reasoning: this changes one expression in THREE functions totalling
-- about 25,000 characters, none of which is otherwise touched, and three
-- hand-copied near-duplicates is three chances to silently drop a line from a
-- report nobody re-reads. Verified before writing this file that the offline
-- versions of all three RPCs (mechanically ported, same names, same v_from/
-- v_to/v_sales/v_cost variable names throughout) contain the exact same two
-- expressions being replaced here.
--
...
-- transcribed by hand.
--
-- IT FAILS LOUDLY IF THE TEXT IS NOT THERE. If a later migration rewrites one
-- of these functions and this file is ever replayed against it, the
-- expression will not match and this raises rather than silently leaving a
-- wrong profit in place. A migration that cannot do its job must not report
-- success.
-- ---------------------------------------------------------------------------
do $$
declare
  ...
  for i in 1 .. array_length(v_targets, 1) by 3 loop
    ...
    if position(v_old_prof in v_src) = 0 then
      raise exception
        ...
        'It has changed since this migration was written - re-apply the fix by hand '
        'rather than leaving profit counting stock bought.', v_target;
    end if;
