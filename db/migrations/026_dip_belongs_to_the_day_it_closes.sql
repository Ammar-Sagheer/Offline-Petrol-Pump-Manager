-- =============================================================================
-- 026_dip_belongs_to_the_day_it_closes.sql
--
...
--
-- ---------------------------------------------------------------------------
-- Ported from the reference app's 039_dip_belongs_to_the_day_it_closes.sql.
-- Verbatim apart from the grant on recalc_stock_checks, which names app_user
-- instead of `authenticated` and has no anon role to revoke from. The bug and
-- the fix are pure arithmetic on stock_checks; nothing here touches identity.
--
-- The reference's 037 and 038 are deliberately not ported: 037 added
-- get_reading_completion() for a strip of day tiles above the nozzle list, and
-- 038 dropped both the strip and the function again. Replaying a function only
-- to delete it would leave this schema no different and the history harder to
-- read, so the pair is collapsed to nothing - which is why the numbering here
-- steps from the reference's 036 to its 039.
-- ---------------------------------------------------------------------------
-- =============================================================================
...
revoke execute on function public.recalc_stock_checks(uuid, date) from public, app_user;
