...
-- 032_treasury.sql
--
-- Ported from the reference app's 044_treasury.sql. Plain Postgres and
-- current_uid()/app_user throughout, same as every migration since 010; no
-- other change to the logic.
--
-- The treasury: the cash actually sitting in the safe on the pump site.
--
-- WHAT THIS MODELS, AND WHY IT IS NOT BANKING. Banking (006/007) is money in a
...
-- as pocket cash. None of that money is in the banking system.
--
-- THIS TABLE IS THAT SHEET. One row per movement, one running balance, and a
-- free-text Details box - the owner's own "Tajori" sheet (Date, Cash In, Cash
-- Out, Balance, Details), in the database. The one thing added on top is a
-- category, so the app can answer "how much has gone out to people this
-- month" without anyone reading thirty lines of prose.
--
...
-- into Banking separately if the owner wants it there. The safe is reconciled
...
--
-- NOT PORTED FROM THE REFERENCE: 045_treasury_opening_entries.sql, which
-- seeds one specific owner's real 36 "Tajori" transactions (names, supplier
-- codes, exact rupee figures). That is one client's private financial history,
-- not a generic starting point - unlike 005_seed_tanks_and_nozzles.sql this
-- has no business being replayed onto every install this app is ever set up
-- for. Every install's treasury starts empty; the owner records "Already in
-- the safe" as the opening entry themselves, same as they would on paper.
-- =============================================================================
...
--
-- NO CAP, unlike bank_transactions (007). The 60-entry cap there was a
-- deliberate trade for a table nobody reads past the last page of. This one
-- replaces a spreadsheet the owner scrolls back through, and losing the detail
-- would lose the point. At five or six lines a day it is under two thousand
-- rows a year.
-- ---------------------------------------------------------------------------
...
-- writing it down". There is exactly one such moment. A second one is always
...
-- bank's no-overdraw rule, which only has to look at one balance, this has to
-- look at the whole chain - an entry inserted or deleted in the middle of the
-- sheet moves every balance after it, so a row that is fine where it lands can
-- still push a later day below zero.
--
...
-- not on each row as it lands. Checking at commit means the only state ever
-- judged is the state that survives.
--
...
-- reason the rest of this app's aggregation is: the page shows a day at a
-- time, and a balance worked out from the rows on screen would be the balance
-- of a page rather than of the safe. The window runs over every row
-- regardless of which ones the reader is looking at.
--
-- security_invoker so the RLS policy below still applies - a view is
-- otherwise read with its owner's rights.
-- ---------------------------------------------------------------------------
...
--
-- Superseded by 033, which changes only where the window ENDS - see that
-- file. Reproduced here at its original 044 shape for the same reason 033
-- reproduces the whole function: a migration that has run is never edited.
-- ---------------------------------------------------------------------------
...
create policy "treasury: super admin only"
  on public.treasury_entries for all to app_user
  using (public.is_super_admin())
  with check (public.is_super_admin());

grant select, insert, update, delete on public.treasury_entries to app_user;
grant select on public.treasury_ledger to app_user;

revoke all on function public.treasury_overview(integer) from public;
grant execute on function public.treasury_overview(integer) to app_user;

-- ---------------------------------------------------------------------------
-- Wire it into the activity log (024, extended by 025 and 026).
--
