...
--
--
-- Ported from the reference app's 041_stock_register.sql. The only edits are
-- the two grants, which name app_user rather than `authenticated` and have no
-- anon role to revoke from. The register's arithmetic reads books_date, which
-- 026 added, so this has to run after it - as it does in the reference.
-- ---------------------------------------------------------------------------
...
-- Same grant shape as every other reporting RPC here: app_user only, and the
-- role check at the top of the body is what actually refuses a staff login.
revoke execute on function public.get_stock_register(date, date) from public;
grant  execute on function public.get_stock_register(date, date) to app_user;

revoke execute on function public.get_range_summary(date, date) from public;
grant  execute on function public.get_range_summary(date, date) to app_user;
