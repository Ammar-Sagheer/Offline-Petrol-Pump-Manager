-- =============================================================================
-- 007_bank_payments.sql
--
-- No account may go negative, and a payment too big for one account can be
-- covered from the others.
--
-- The rule is per account: each one must stand up on its own. Refusing more
-- often needs a way through, which is what record_bank_payment() is for. A
-- payment larger than the chosen account takes what that account has and draws
-- the rest from the other accounts the owner picked, in the order he picked
-- them, writing one transaction per account.
--
...
--
-- Ported from the reference app's 020 and 021, collapsed to 021's final
-- per-account rule (020's combined-balance version existed only briefly in
-- that app's own history and is not worth replaying here), with auth.uid()
-- replaced by current_uid().
-- =============================================================================
...
-- ---------------------------------------------------------------------------
-- Per-account guard: refuses a payment insert that would draw an account
-- below zero.
-- ---------------------------------------------------------------------------
  ...
  for v_part in select * from jsonb_array_elements(v_parts) loop
    ...
    values
      ((v_part ->> 'account_id')::uuid, 'payment', (v_part ->> 'amount')::numeric,
       p_date, v_category, v_note, public.current_uid());
  end loop;
...
revoke execute on function public.bank_account_balance(uuid) from public;
revoke execute on function public.record_bank_payment(uuid, numeric, date, text, text, uuid[]) from public;
grant  execute on function public.bank_account_balance(uuid) to app_user;
grant  execute on function public.record_bank_payment(uuid, numeric, date, text, text, uuid[]) to app_user;
