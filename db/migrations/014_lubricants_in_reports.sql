...
--
-- Ported from the reference app's 025_lubricants_in_reports.sql. The four RPCs
-- it replaces were checked line by line against the versions 008 left behind
-- before this was written: every difference is a lubricant addition, so these
-- bodies are the reference's, not a re-derivation. Two deliberate departures:
-- policies and grants target app_user (see 004), and the explicit WHERE TRUE on
-- every statement in reset_all_data is dropped - that exists only because
-- Supabase preloads the safeupdate extension, which refuses a WHERE-less UPDATE
-- or DELETE. There is no safeupdate here.
--
...
-- The dashboard's day
--
-- Body is 008's, with the lubricant block added at the end.
-- ---------------------------------------------------------------------------
...
-- exactly the busywork the reset exists to avoid.
--
-- The reference app writes an explicit WHERE TRUE on every statement here,
-- because Supabase preloads the safeupdate extension, which refuses a
-- WHERE-less UPDATE or DELETE. Nothing preloads safeupdate on a local cluster,
-- so those are dropped - the same call this repo made for the rest of 008.
--
-- The ledger's delete trigger is switched off around its one DELETE. Nothing
-- else in the app may empty the ledger, and the trigger is put straight back.
-- ---------------------------------------------------------------------------
...
begin
  ...
  delete from public.lubricant_sales;
  delete from public.lubricant_purchases;
