...
--   * the ledger physically refuses updates and deletes, with one narrow
--     exception for created_by/credit_sale_id going to null (see 001's comments
--     on ledger_entries for why those two FKs are ON DELETE SET NULL)
--   * tank stock is recomputed from history after every relevant write
--
-- Ported from the reference app's 002_functions_and_triggers.sql, 006 (grant
-- lock-down - folded into 004_rls_policies.sql here) and 011 (the append-only
-- exception for created_by), with auth.uid() replaced by current_uid()
-- throughout - see 002_identity_and_sessions.sql for why.
-- =============================================================================
...
--
-- RLS alone is not enough here: application code always connects through one
-- role, and a bug there could otherwise rewrite what a customer owes. A
-- trigger cannot be bypassed that way - only a superuser session running
-- outside the app could disable it, which is not something a request handler
-- can do. Corrections are made by posting a new opposite entry, which leaves
-- the original visible - that is the point.
--
-- The one permitted UPDATE: created_by or credit_sale_id being cleared when the
-- profile or credit slip they point at is deleted (both are ON DELETE SET NULL
-- - see 001). Compared as whole rows minus those two columns, so adding a
-- column later cannot silently widen what this allows.
-- ---------------------------------------------------------------------------
