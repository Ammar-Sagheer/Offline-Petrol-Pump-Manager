...
--
-- The audit trail: one trigger on sixteen tables writing who changed what, and
-- an append-only log to hold it.
--
-- Ported from the reference app's 035_activity_log.sql. The reference README
-- calls this the hardest single thing to port, and on a different database it
-- would be - it is PL/pgSQL reading a row as jsonb, diffing old against new and
-- building an English sentence, none of which exists in SQLite. On real
-- Postgres it is none of those things. Three edits, all the usual ones:
--
--   * activity_actor() reads auth.uid() -> public.current_uid(), the same
--     session-variable swap 002 made for everything else. This is the one
--     function the reference singles out as needing an offline equivalent.
--   * policies and grants target app_user rather than `authenticated`.
--   * the `revoke all ... from anon` line is dropped: there is no anon role
--     here, and a request with no session never reaches a query at all (004).
--
-- Everything else - to_jsonb on the row, jsonb_object_keys, the field-by-field
-- diff, the append-only guard, the sixteen triggers - applies verbatim.
--
-- WHY KEEP IT AT ALL on a single-user machine, where "who did this" has one
-- answer: because here it does not. This build has staff logins with a
-- data_entry role, the whole reason those roles exist is that somebody other
-- than the owner types the day in, and the log is the only place recording
-- which of them did.
-- =============================================================================
...
as $$
  select (select public.current_uid()),
         coalesce(
           (select p.full_name from public.profiles p where p.id = (select public.current_uid())),
           case when (select public.current_uid()) is null then 'System' else 'Unknown login' end
         );
$$;
...
create policy "activity_log: super admin reads"
  on public.activity_log for select to app_user
  using (public.is_super_admin());

revoke execute on function public.activity_actor()      from public;
revoke execute on function public.trg_write_activity()  from public;
grant select on public.activity_log to app_user;
