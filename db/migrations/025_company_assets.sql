...
--
-- Ported from the reference app's 036_company_assets.sql. Only the usual two
-- edits: policies and grants target app_user rather than `authenticated`, and
-- the `revoke ... from anon` lines are dropped because there is no anon role
-- here (004). The table, the enum, the summary RPC and the activity-log
-- branch are all verbatim - none of them touch identity.
--
-- The trg_write_activity replacement below is reproduced in full because
-- `create or replace function` replaces the whole body; it is byte-identical
-- to the one 024 installed apart from the new `company_assets` branch.
-- =============================================================================
...
revoke execute on function public.get_company_assets_summary() from public;
grant  execute on function public.get_company_assets_summary() to app_user;
...
create policy "company assets: super admin only"
  on public.company_assets for all to app_user
  using (public.is_super_admin())
  with check (public.is_super_admin());

grant select, insert, update, delete on public.company_assets to app_user;
