-- =============================================================================
-- 020_remove_a_customer.sql
--
-- Ported from the reference app's 031_remove_a_customer.sql. Plain Postgres;
-- the only edits are the grants at the bottom targeting app_user (see 004).
--
-- Taking a customer off the list - a name typed wrong, a duplicate, or an
-- account that has genuinely finished.
--
-- THE SAME TWO MEANINGS AS delete_lubricant (013), for the same
-- reason:
--
...
-- (003, widened by 013) is about CORRECTING an entry, which is still forbidden; this
...
revoke execute on function public.delete_customer(uuid) from public;
grant  execute on function public.delete_customer(uuid) to app_user;
...
revoke execute on function public.get_retired_customers() from public;
grant  execute on function public.get_retired_customers() to app_user;
